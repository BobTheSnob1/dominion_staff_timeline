...
def plot_staff_timeline():
    ...
    role_times = input(
        "Would you like to add numbers for time spent in each role? [y/n]: "
    )  # noqa: E501
    ...
    for index, staff_member in tqdm(
        enumerate(df.columns), total=len(df.columns), desc="Processing:"
    ):
        ...
        if start_date and current_role and colors[current_role] != "none":
            ...
            ax.barh(staff_member, duration, left=start_date, color=colors[current_role])
            if role_times:
                ax.text(
